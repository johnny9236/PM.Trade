    ...
    public class CServerSocket
    {
        ...
        private void OnDataReceived(IAsyncResult asyn)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Decoder d = Encoding.GetEncoding("gb2312").GetDecoder(); //Encoding.UTF8.GetDecoder();
        ...
        public bool SendText(string mens, int SocketIndex)
        {
            if ((Clients.Count - 1) >= SocketIndex)
            {
                ...
                try
                {
                    byte[] byData = System.Text.Encoding.GetEncoding("gb2312").GetBytes(mens);
        ...
        public bool SendFile(string FileName, string PreString, string PosString, int SocketIndex)
        {
            if ((Clients.Count - 1) >= SocketIndex)
            {
                ...
                try
                {
                    byte[] preBuf = Encoding.GetEncoding("gb2312").GetBytes(PreString);// Encoding.UTF8.GetBytes(PreString);
                    byte[] postBuf = Encoding.GetEncoding("gb2312").GetBytes(PosString);
