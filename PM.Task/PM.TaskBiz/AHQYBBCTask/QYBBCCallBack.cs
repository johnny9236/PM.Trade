...
{
    public class QYBBCCallBack : ITimerTaskCallBack
    {
        /// <summary>
        /// 回调任务
        /// </summary>
        ...
        private bool IncrementToDB(List<BBCQueryInfo> modelList)
        {
            ...
            if (rtn)
            {
                ...
                {
                    LogTxt.WriteEntry("明细入账异常" + ex.Message, "青阳匹配");
                }
        ...
        {
            #region  回调准备
            var urlStr = ConfigHelper.GetCustomCfg("QY", "BusinessUrl");
            var enCodingStr = ConfigHelper.GetCustomCfg("QY", "enCoding");
            var chkStr = ConfigHelper.GetCustomCfg("QY", "chkStr");//核对值
            ...
            {
                LogTxt.WriteEntry("回调地址或者编码等未设置，请设置", "青阳匹配");
                return;
            }
            ...
            var dbEnter = new PM.TaskBiz.ORM.Pub_Entities();
            var matchList = dbEnter.T_Margin.Where(p => (p.Match != 1 || p.Match == null)&&p.BankType.ToLower()=="BBC".ToLower());//获取匹配表待匹配信息 (获取借的标记   0借  1贷)
            ...
            foreach (var lst in matchList)//匹配
            {
                ...
                LogTxt.WriteEntry("回调信息" + urlStr + postStr, "青阳匹配回调");
                ...
                {
                    LogTxt.WriteEntry("回调匹配响应失败" + postStr, "青阳匹配回调");
                }
            }
            if (haveMatch)//先保存一次
            {
                ...
                {
                    LogTxt.WriteEntry("订单匹配处理异常" + ex.Message, "青阳匹配回调");
                }
            }
            #endregion
        }
