...
namespace PM.TaskBiz.HYBOCTASK
{
    ...
    public class BOCCall : ITimerTaskCallBiz
    {
        ...
        private void Call(bool IsToday, string token)
        {
            ...
            //test
            sectionList = new List<ORM.T_Pay_VirtualAccount>();
            var vr = new PM.TaskBiz.ORM.T_Pay_VirtualAccount();
            vr.VirtualAccount ="377958654978";
            sectionList.Add(vr);
            //
            ...
            foreach (var section in sectionList)
            {
                ...
                requestQuery.RecNum = "0"+pageRecordCount.ToString();
                requestQuery.Actacn = section.VirtualAccount;//账号
                ...
                if (!string.IsNullOrEmpty(queryRes.RspCod) && (queryRes.RspCod.ToLower() == "B001".ToLower() || queryRes.RspCod.ToLower() == "B002".ToLower() && queryRes.AccountDtlList.Count > 0))//成功
                {
                    ...
                    queryList_temp.AddRange(queryRes.AccountDtlList);
                    #region 分页
