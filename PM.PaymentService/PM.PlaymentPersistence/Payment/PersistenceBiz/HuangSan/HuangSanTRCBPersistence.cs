...
namespace PM.PlaymentPersistence.Payment.PersistenceBiz.HuangSan
{
    ...
    ///  农商行退款处理（包括人工）
    /// </summary>
    public class HuangSanTRCBPersistence
    {
        ...
        public PayRefundModel DoRefundPay(PayRefundModel payRefundMode)
        {
            ...
            HSanTRCBRefundRequset refundModel = null;
            HSanTRCBRefundInfo qyRefund = null;
            if (null != payRefundMode && null != payRefundMode.PayRefundDtl && payRefundMode.PayRefundDtl.Count > 0)
            {
                ...
                //{
                //    LogTxt.WriteEntry(string.Format("无对应标段入账数据授权号{0}标段号{1}", authCode, sectionNo), "农商行退款异常");
                //    return new PayRefundModel();//退回
                //}

                #region  发送准备
                refundModel = new HSanTRCBRefundRequset();
                ...
                refundModel.HSanTRCBRefundList = new List<HSanTRCBRefundInfo>();
                if (null != payRefundMode && null != payRefundMode.PayRefundDtl && payRefundMode.PayRefundDtl.Count > 0)
                {
                    foreach (var refundPay in payRefundMode.PayRefundDtl)
                    {
                        qyRefund = new HSanTRCBRefundInfo();
                        ...
                        {
                            LogTxt.WriteEntry("无入账账号,对应主订单号:" + payRefundMode.OrderNo + "子订单号:" + refundPay.OrderNo, "农商行退款");
                            continue;
                        }
                        var seqNum = refundPay.PaySettingAccNo;//GetRtnSeqNum(rtnList, refundPay.ReceiptAcountNo);
                        if (string.IsNullOrEmpty(seqNum))
                        {
                            LogTxt.WriteEntry("无原入账流水号,对应主订单号:" + payRefundMode.OrderNo + "子订单号:" + refundPay.OrderNo, "农商行退款");
                            continue;
                        }
                        ...
                        if (CreateOrderAndBusniess(seqNum, payRefundMode, refundPay))//订单相关信息创建成功后添加记录
                            refundModel.HSanTRCBRefundList.Add(qyRefund);
                    }
                #endregion
                    //提交数据
                    try
                    ...
                    {
                        LogTxt.WriteEntry("异常" + ex.Message, "农商行退款异常");
                        return new PayRefundModel();//退回
                    }

                    //接口通讯
                    HSanTRCBRefundResponse refundRtn = Manager.PaymentManager(refundModel);//发送协议
                    if (null != refundRtn && null != refundRtn.TRCBReturnRefundDtlList && refundRtn.TRCBReturnRefundDtlList.Count > 0)
                    {
                        // 返回处理
        ...
        /// <returns></returns>
        private PayRefundModel SetOrderAndBusniess(string mainOrderNo, HSanTRCBRefundResponse refundRtn)
        {
            ...
            if (null != refundRtn && null != refundRtn.TRCBReturnRefundDtlList && refundRtn.TRCBReturnRefundDtlList.Count > 0)
            {
                try
                {
                    foreach (var bank in refundRtn.TRCBReturnRefundDtlList)
                    {
                        ...
                        {
                            LogTxt.WriteEntry(string.Format("退款响应, 订单号 为空 信息为[{0}]银行流水[{1}] ", bank.InAcctNo, bank.HstSeqNum), "农商行退款记录");
                            continue;
                        }
                        ...
                        if (null != orderInfo)
                        {
                            ...
                            if (bank.Result == "1")
                            {
                                ...
                                {
                                    LogTxt.WriteEntry(string.Format("退款响应,未找到对应入账信息（T_AHQY）银行流水=[{0}] 主订单号=[{1}] ", bank.HstSeqNum, mainOrderNo), "农商行退款记录");
                                    //   continue;
                                }
                            }
                            ...
                            {
                                LogTxt.WriteEntry(string.Format("退款响应,未找到对应业务信息（T_Pay_OrderBusiness）订单号=[{0}] 主订单号=[{1}]", orderInfo.OrderNo, mainOrderNo), "农商行退款记录");
                                continue;
                            }
                        }
                        else
                        {
                            LogTxt.WriteEntry(string.Format("退款响应,未找到对应订单信息（T_Pay_Order）银行流水=[{0}] 主订单号=[{1}] ", bank.HstSeqNum, mainOrderNo), "农商行退款记录");
                            continue;
                        }
        ...
        private string GetRtnSeqNum(List<T_AHQY> models, string accountNo)
        {
            ...
            if (null != IncomeAccount)
            {
                ...
                {
                    LogTxt.WriteEntry(string.Format("账号{0} 标段号{1}原来处理成功ID{2}", accountNo, IncomeAccount.SectionCode, IncomeAccount.ID), "农商行退款未获取银行流水记录");
                }
                else
                {
            ...
            {
                LogTxt.WriteEntry(string.Format("账号{0}未找到", accountNo), "农商行退款未获取银行流水记录");
            }
            return seqNum;
        }
        #endregion
