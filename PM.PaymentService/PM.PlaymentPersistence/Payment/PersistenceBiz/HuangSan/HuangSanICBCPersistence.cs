...
namespace PM.PlaymentPersistence.Payment.PersistenceBiz.HuangSan
{
    ...
    /// 工行退款（无人工）
    /// </summary>
    public class HuangSanICBCPersistence
    {
        ...
        public PayRefundModel DoRefundPay(PayRefundModel payRefundMode)
        {
            ...
            HSICBCRefundRequset refundModel = null;
            HSICBCRefundInfo qyRefund = null;
            if (null != payRefundMode && null != payRefundMode.PayRefundDtl && payRefundMode.PayRefundDtl.Count > 0)
            {
                ...
                //{
                //    LogTxt.WriteEntry(string.Format("无对应标段入账数据授权号{0}标段号{1}", authCode, sectionNo), "工行退款异常");
                //    return new PayRefundModel();//退回
                //}

                #region  发送准备
                refundModel = new HSICBCRefundRequset();
                ...
                refundModel.SeqNo = payRefundMode.MainAccount;//虚拟账号(特殊注意)
                refundModel.ICBCRefundList = new List<PaymentProtocolModel.BankCommModel.ICBCRefundInfo>();
                if (null != payRefundMode && null != payRefundMode.PayRefundDtl && payRefundMode.PayRefundDtl.Count > 0)
                {
                    foreach (var refundPay in payRefundMode.PayRefundDtl)
                    {
                        qyRefund = new HSICBCRefundInfo();
                        ...
                        {
                            LogTxt.WriteEntry("无入账账号,对应主订单号:" + payRefundMode.OrderNo + "子订单号:" + refundPay.OrderNo, "工行退款");
                            continue;
                        }
                        var seqNum = refundPay.PaySettingAccNo;//GetRtnSeqNum(rtnList, refundPay.ReceiptAcountNo);
                        if (string.IsNullOrEmpty(seqNum))
                        {
                            LogTxt.WriteEntry("无原入账流水号,对应主订单号:" + payRefundMode.OrderNo + "子订单号:" + refundPay.OrderNo, "工行退款");
                            continue;
                        }
                        ...
                        if (CreateOrderAndBusniess(seqNum, payRefundMode, refundPay))//订单相关信息创建成功后添加记录
                            refundModel.ICBCRefundList.Add(qyRefund);
                    }
                #endregion
                    //提交数据
                    try
                    ...
                    {
                        LogTxt.WriteEntry("异常" + ex.Message, "工行退款异常");
                        return new PayRefundModel();//退回
                    }

                    //接口通讯
                    HSICBCRefundResponse refundRtn = Manager.PaymentManager(refundModel);//发送协议
                    if (null != refundRtn && null != refundRtn.ICBCReturnRefundDtlList && refundRtn.ICBCReturnRefundDtlList.Count > 0)
                    {
                        // 返回处理
        ...
        /// <returns></returns>
        private PayRefundModel SetOrderAndBusniess(string mainOrderNo, HSICBCRefundResponse refundRtn)
        {
            ...
            if (null != refundRtn && null != refundRtn.ICBCReturnRefundDtlList && refundRtn.ICBCReturnRefundDtlList.Count > 0)
            {
                try
                {
                    foreach (var bank in refundRtn.ICBCReturnRefundDtlList)
                    {
                        ...
                        {
                            LogTxt.WriteEntry(string.Format("退款响应, 订单号 为空 信息为[{0}]银行流水[{1}] ", bank.InAcctNo, bank.HstSeqNum), "工行退款记录");
                            continue;
                        }
                        ...
                        if (null != orderInfo)
                        {
                            ...
                            if (bank.Result == "1")
                            {
                                ...
                                {
                                    LogTxt.WriteEntry(string.Format("退款响应,未找到对应入账信息（T_AHQY）银行流水=[{0}] 主订单号=[{1}] ", bank.HstSeqNum, mainOrderNo), "工行退款记录");
                                    //   continue;
                                }
                            }
                            ...
                            {
                                LogTxt.WriteEntry(string.Format("退款响应,未找到对应业务信息（T_Pay_OrderBusiness）订单号=[{0}] 主订单号=[{1}]", orderInfo.OrderNo, mainOrderNo), "工行退款记录");
                                continue;
                            }
                        }
                        else
                        {
                            LogTxt.WriteEntry(string.Format("退款响应,未找到对应订单信息（T_Pay_Order）银行流水=[{0}] 主订单号=[{1}] ", bank.HstSeqNum, mainOrderNo), "工行退款记录");
                            continue;
                        }
        ...
        private string GetRtnSeqNum(List<T_AHQY> models, string accountNo)
        {
            ...
            if (null != IncomeAccount)
            {
                ...
                {
                    LogTxt.WriteEntry(string.Format("账号{0} 标段号{1}原来处理成功ID{2}", accountNo, IncomeAccount.SectionCode, IncomeAccount.ID), "工行退款未获取银行流水记录");
                }
                else
                {
            ...
            {
                LogTxt.WriteEntry(string.Format("账号{0}未找到", accountNo), "工行退款未获取银行流水记录");
            }
            return seqNum;
        }
        #endregion
