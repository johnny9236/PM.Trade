...
namespace PM.PaymentModel.BizModel.AHQY
{
    ...
    /// 青阳虚拟账号创建
    /// </summary>
    public class QYVirtualAccountRequset : QYCommBase
    {
        /// <summary>
        /// 交易代码
        /// </summary>
        ...
        public string GetMessagePaket()
        {
            ...
            sb.Append("<CenterNo>{1}</CenterNo>");
            sb.Append("<TransDate>{2}</TransDate>");
            sb.Append("<TransTime>{3}</TransTime>");
            sb.Append("<SeqNo>{4}</SeqNo>");
            sb.Append("</head>");
            sb.Append("<body>");
            sb.Append("<AcctNo>{5}</AcctNo>");
            sb.Append("<ProjectNo>{6}</ProjectNo>");
            sb.Append("<BiaoDuanNo>{7}</BiaoDuanNo>");
            sb.Append("<ProjectName>{8}</ProjectName>");
            sb.Append("<OpenDate>{9}</OpenDate>");
            sb.Append("<OpenTime>{10}</OpenTime>");
            sb.Append("<IsRetire>{11}</IsRetire>");
            sb.Append("<MatuDay>{12}</MatuDay>");
            sb.Append("</body>");
            sb.Append("</root>");
    ...
    /// 虚拟账号创建返回
    /// </summary>
    public class QYVirtualAccountResponse
    {
        /// <summary>
        /// 交易代码
        /// </summary>
        ...
        public bool GetModel(string packetString)
        {
            ...
            try
            {
                ...
                var xdoc = XDocument.Parse(packetString);//是否需要12位去掉
