...
namespace PM.HSanTRCBPtlBiz
{
    ...
    public partial class HSanTRCBCommonProtocols
    {
        ...
        {
            LogTxt.WriteEntry(string.Format("发送报文--{0}", sendMessage), "农商行创建虚拟账号协议报文");
            var rtnStr = Send(sendMessage, cfgInfo);
            LogTxt.WriteEntry(string.Format("接受报文--{0}", rtnStr), "农商行创建虚拟账号协议报文");
            return rtnStr;
        }
        /// <summary>
        /// 更新开标时间
        ...
        {
            LogTxt.WriteEntry(string.Format("发送报文--{0}", sendMessage), "农商行通用协议报文");
            ...
            LogTxt.WriteEntry(string.Format("接受报文--{0}", returnStr), "农商行通用协议报文");
            return returnStr;
        }
