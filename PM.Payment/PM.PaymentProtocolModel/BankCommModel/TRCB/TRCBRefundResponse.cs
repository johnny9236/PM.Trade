...
namespace PM.PaymentProtocolModel.BankCommModel.TRCB
{
    ...
    /// 退款返回
    /// </summary>
    public class TRCBRefundResponse
    {
        /// <summary>
        /// 交易代码
        /// </summary>
        ...
        /// 保证金退回返回列表
        /// </summary>
        public List<TRCBReturnRefundDtl> TRCBReturnRefundDtlList { get; set; }
        /// <summary>
        /// 获取明细对象
        /// </summary>
        ...
        public virtual bool GetModel(string packetString)
        {
            ...
            try
            {
                ...
                if (bankList != null && bankList.Count() > 0)
                    this.TRCBReturnRefundDtlList = new List<TRCBReturnRefundDtl>();
                foreach (var bank in bankList)
                {
                    var dtl = new TRCBReturnRefundDtl();
                    dtl.HstSeqNum = bank.HstSeqNum;
                    dtl.InAcctNo = bank.InAcctNo;
                    dtl.InName = bank.InName;
                    dtl.Result = bank.Result;

                    this.TRCBReturnRefundDtlList.Add(dtl);
                }
            }
            catch (Exception ex)
            {
    ...
    /// 保证金退回返回
    /// </summary>
    public class TRCBReturnRefundDtl
    {
        /// <summary>
        /// 转账流水号
        /// </summary>
